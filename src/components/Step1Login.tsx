...
export default function Step1Login({ onLogin }: Step1LoginProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Cek Kredensial Super Admin / Owner Bawaan
      if (cleanUsername === 'owner' && pin === '1234') {
        onLogin('owner', 'yos.krisnawan@anymindgroup.com', 'owner');
        return;
      }
      if (cleanUsername === 'spv.lead' && pin === '1234') {
        onLogin('spv', 'spv@anymindgroup.com', 'spv.lead');
        return;
      }
      ...
      if (docSnap.exists()) {
        ...
        if (userData.pin === pin) {
          onLogin('counter', userData.email || `${cleanUsername}@anymindgroup.com`, cleanUsername);
        } else {
